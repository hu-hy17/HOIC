...
def eval_hand_acc():
    ...
    ref_kps = os.path.join(tps_root_dir, obj_name, 'Ours/' + view_dir + '/ref_tps.npy')
    ref_kps = np.load(ref_kps)
    sa22_kps = os.path.join(tps_root_dir, obj_name, 'SA22/' + view_dir + '/ref_tps.npy')
    sa22_kps = np.load(sa22_kps)
    ...
    for i, fn in enumerate(label_file_list):
        ...
        for s in labels['shapes']:
            ...
            ref = ref_kps[frame_id][tip_idx]
            sa22 = sa22_kps[frame_id][tip_idx]
            ours = ours_kps[frame_id][tip_idx]
            pixel_err[0] += np.linalg.norm(ref - gt)
            pixel_err[1] += np.linalg.norm(sa22 - gt)
...
def eval_obj_acc():
    mask_root = 'results/quanti/Data/Mask/{}/Ours/{}'.format(obj_name, view_dir)
    sa22_mask_root = 'results/quanti/Data/Mask/{}/SA22/{}'.format(obj_name, view_dir)
    label_root = 'results/quanti/Label/Mask/{}/{}/Masks'.format(obj_name, view_dir)
    mask_file_list = glob.glob(mask_root + '/*.png')
    ...
    for fn in label_file_list:
        ...
        start_idx = 0
        end_idx = len(mask_file_list) // 2

        if frame_id <= start_idx or frame_id >= end_idx:
            continue

        ref_mask_file = mask_root + '/ref_{:04d}.png'.format(frame_id)
        ours_mask_file = mask_root + '/mimic_{:04d}.png'.format(frame_id)
        sa22_mask_file = sa22_mask_root + '/ref_{:04d}.png'.format(frame_id)
        ref_mask = cv2.imread(ref_mask_file)
        ours_mask = cv2.imread(ours_mask_file)
        sa22_mask = cv2.imread(sa22_mask_file)

        label_mask = cv2.imread(fn)

        IoU = np.zeros(3)

        IoU[0] = cal_IoU(label_mask, ref_mask)
        IoU[1] = cal_IoU(label_mask, sa22_mask)
