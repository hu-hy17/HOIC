# Source: https://github.com/ARISE-Initiative/robosuite/blob/master/robosuite/models/base.py
import io
import os
import xml.dom.minidom
import xml.etree.ElementTree as ET

from uhc.data_loaders.mjxml.mjcf_utils import (add_prefix, find_elements,
                                               recolor_collision_geoms, sort_elements,
                                               string_to_array)
...
class MujocoXML(object):
    ...
    def __init__(self, fname):
        ...
        # Commented By HuHaoyu because it can not handle childclass properly
    ...
    def merge(self, others, merge_body="default"):
        ...
        for idx, other in enumerate(others):
            if not isinstance(other, MujocoXML):
                raise ValueError("{} is not a MujocoXML instance.".format(
                    type(other)))
...
class MujocoModel(object):
    ...
    def correct_naming(self, names):
        ...
        if type(names) is str:
            return self.naming_prefix + names if not self.exclude_from_prefixing(
                names) else names
        elif type(names) is list:
            return [
                self.naming_prefix +
                name if not self.exclude_from_prefixing(name) else name
                for name in names
            ]
    ...
    def set_sites_visibility(self, sim, visible):
        ...
        for vis_g in self.sites:
            ...
            if (visible and sim.model.site_rgba[vis_g_id][3] < 0) or \
                ...
                sim.model.site_rgba[vis_g_id][
                    3] = -sim.model.site_rgba[vis_g_id][3]
