...
# SingleDepth Dataset
class DatasetSingleDepth:
    def __init__(self, model_fn, data_spec, noise=0.0, trans=(0, 0), mode='train', idx=None, step=1):
        """
        ...
        """
        logger.info("Reading Expert Data From {}".format(data_spec.get("seq_name")))
        ...
        if self.with_obj:
            ...
            self.model = mjpy.load_model_from_path(self.model_fn)
        ...
        if idx is None:
            self.all_seqs = [self.all_seqs[i] for i in range(0, len(self.all_seqs), step)]
            for seq in tqdm.tqdm(self.all_seqs):
                self.preprocess_seq(seq)
        else:
            self.preprocess_seq(self.all_seqs[idx])
    ...
    def compute_contact_info(self, hand_dof_seq, obj_pose_seq, seq_len):
        ...
        for i in range(seq_len):
            sim.data.qpos[:self.hand_dof] = hand_dof_seq[i]
            sim.data.qpos[self.hand_dof:] = obj_pose_seq[i]
            sim.forward()
    ...
    def compute_body_pos_quat_from_seq(self, hand_dof_seq, seq_len):
        ...
        for i in range(seq_len):
            ...
            sim.forward()
            body_pos_seq[i] = sim.data.body_xpos[self.hand_body_idx]
            body_quat_seq[i] = sim.data.body_xquat[self.hand_body_idx]
...
if __name__ == '__main__':
    ...
    dataset = DatasetSingleDepth(model_fn, data_spec)
    dataset.show_expert()
