...
class MotionData:
    def __init__(self):
        ...
        self.surface_contact_force = []
        self.rest_obj_ft = []


# Change root action
class HandObjMimic4(mujoco_env.MujocoEnv):
    def __init__(self, cfg, expert_seq, model_xml, data_specs, mode="train"):
        self.sim_step = cfg.sim_step
        mujoco_env.MujocoEnv.__init__(self, model_xml, self.sim_step)
        ...
        self.fk_sim = mujoco_py.MjSim(self.model)
    ...
    def get_noise_pose(self, qpos_arr, obj_pose_arr):
        ...
        for i in range(frame_num):
            ...
            self.fk_sim.data.qpos[:] = np.concatenate([qpos, obj_pose])
            self.fk_sim.forward()
            hand_kp = self.fk_sim.data.body_xpos[self.hand_body_idx].copy()
            root_quat = self.fk_sim.data.body_xquat[self.hand_body_idx[0]].copy()
    ...
    def compute_data_frame(self):
        ...
        self.motion_data.hand_qpos.append(self.get_hand_qpos())
    ...
    def get_contact(self):
        ...
        for i, contact in enumerate(self.data.contact[:self.data.ncon]):
            ...
            if self.hand_geom_range[0] <= g1 <= self.hand_geom_range[1] and \
                ...
                mjf.mj_contactForce(self.model, self.data, i, c_array)
    ...
    def solve_rfc(self):
        ...
        if not self.cc_cfg.explain_force:
            return self.obj_vf, self.obj_vt, np.linalg.norm(self.obj_vf) + w_t * np.linalg.norm(self.obj_vt)
        ...
        # Expand Contact
        e_contact_num = 5 if self.cc_cfg.surface_contact else 1
        ...
        # Surface Balance Term
        J_b = np.zeros((4 * n_c, 4 * n_c))
        if self.cc_cfg.surface_contact:
            for fi in range(n_c // 5):
                for j in range(5):
                    c = np.zeros(4 * n_c)
                    c[20 * fi: 20 * (fi + 1)] = -1
                    c[20 * fi + 4 * j: 20 * fi + 4 * (j + 1)] = 4
                    c = c[:, np.newaxis]
                    J_b += np.matmul(c, c.T)

        # Solve QP
        # qpsolvers
        Q = 2 * (np.matmul(J_f.T, J_f) + w_t * np.matmul(J_t.T, J_t) + w_balance * J_b) + 1e-7 * np.eye(4 * n_c)
        ...
        if self.mode == 'test':
            # save result
            self.motion_data.surface_contact_frames.append(np.stack(e_cps))
            force_arr = []
            for i in range(n_c):
                f = np.matmul(A_arr[i], res[4 * i: 4 * (i + 1)])
                force_arr.append(f)
            self.motion_data.surface_contact_force.append(np.stack(force_arr))
            self.motion_data.rest_obj_ft.append(np.concatenate([rest_force, rest_torque]))
